...
import DatePicker from "./InDate";
import { useRouter } from "next/navigation";
import { headers } from "next/headers";
import { BtnLoader } from "./buttonLoader";

interface CardProps {
    rollNumber: string | null; 
    type: "outpass" | "homepass";
}
...
export const CardPass = ({ rollNumber, type }: CardProps) => {
    ...
    const Create=async ()=>{
           setLoader(true)
            if (type==="outpass") {
                ...
                try {
                    await axios.post(`http://localhost:3000/api/homepass?rollNo=${rollNumber}` ,{
                        headers:{
                            Indate:Indate
                        }
                    })
            ...
            <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-gray-900 to-gray-700">
                <div className="bg-gray-800 border border-gray-600 rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
                    <div className="flex flex-col items-center">
                        <svg className="w-8 h-8 text-gray-200 animate-spin fill-blue-600 mb-4" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex justify-center items-start min-h-screen bg-gradient-to-r from-gray-900 to-gray-700 pt-10">  
            <div className="bg-gray-800 border border-gray-600 rounded-lg shadow-lg p-6 max-w-md w-full mx-4">  
                ...
                <div className="text-gray-300 mb-4">  
                    ...
                    {type === "homepass" && (  
                        <div className="mb-4">  
                            <p><strong>Indate:</strong><DatePicker onDateSelect={(selectedDate)=>{setIndate(selectedDate)}} /></p>  
                        </div>  
                    
                    )}  
                </div>  
    
                <button onClick={Create} className="w-full text-white bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-md px-5 py-2.5 mt-4">  
                {loader ? <BtnLoader /> : ("Create " + (type === "outpass" ? "Outpass" : "Homepass"))}
                </button>  
            </div>  
        </div>  
    );
